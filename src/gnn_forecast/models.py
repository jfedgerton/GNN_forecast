...
except Exception as e:  # optional dependency gate
    GATConv = GCNConv = SAGEConv = None
    _TORCH_GEOM_IMPORT_ERROR = e


class _MissingTorchGeometric(nn.Module):
    def __init__(self):
        super().__init__()
        raise ImportError(
            "torch_geometric is required for GNN models. "
            f"Original error: {_TORCH_GEOM_IMPORT_ERROR}"
        )
